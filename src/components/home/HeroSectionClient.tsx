...
const HeroSectionClient = ({ profile }: HeroSectionClientProps) => {
  // Fallback values if profile is not loaded
  const name = profile?.name || 'Nyi Htut Zaw';
  const title = profile?.title || 'Senior Software Engineer';
  const bio = profile?.bio || 'With extensive experience in web, mobile, and AI-powered applications, I deliver robust solutions across diverse technology stacks.';
  const email = profile?.email || 'nyihtutzaw.2015@gmail.com';
  const linkedinUrl = profile?.linkedin_url || 'https://www.linkedin.com/in/nyi-htut-zaw-9b741115a/';
  const githubUrl = profile?.github_url || 'https://github.com/nyihtutzaw';
  const profileImage = profile?.profile_image_url || '/profile.png';
