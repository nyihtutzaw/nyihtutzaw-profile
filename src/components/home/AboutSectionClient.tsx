...
const AboutSectionClient = ({ about, coreValues, timelineEvents }: AboutSectionClientProps) => {
  ...
  // Fallback values
  const introTitle = about?.intro_title || 'More Than Just Code';
  const introParagraphs = about?.intro_paragraphs || [
    'I believe technology should serve humanity. Throughout my career, I\'ve focused on creating solutions that make a real difference.',
    'Based in Bangkok, Thailand, I bring a global perspective to every project.'
  ];
        ...
        <div className={`grid md:grid-cols-2 gap-8 lg:gap-12 mb-12 sm:mb-20 transition-all duration-1000 delay-300 transform ${
          ...
          <div className="grid grid-cols-1 gap-4">
            {coreValues.map((value, index) => {
              const Icon = iconMap[value.icon] || LightBulbIcon;
              return (
                <div 
                  key={value.id}
          ...
          <div className="relative">
            ...
            <div className="space-y-8 sm:space-y-12">
              {timelineEvents.map((event, index) => {
                const Icon = iconMap[event.icon] || RocketLaunchIcon;
